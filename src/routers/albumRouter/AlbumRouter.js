...
AlbumRouter.get('/recommend', (req, res, next) => {
  ...
  AlbumModel.find(
    {},
    { release_company: 0, album_introduction: 0 },
    { skip: (pageNum - 1) * pageSize, limit: pageSize },
  )
    .then((album) => {
      album.forEach((_, index) => {
        SingerModel.findById(album[index].singer_id, { singer_name: 1 }).then(
          (singer) => {
            album[index]._doc.singer_name = singer.singer_name
            album[index].album_img =
              '/imgs/music_and_album_imgs/' + album[index].album_img
            data.push({
              ...album[index]._doc,
            })
          },
      ...
      setTimeout(() => {
        res.send({
          status: 1,
          data: { totalCount: album.length, albumArr: data },
        })
      }, 100)
    })
    .catch((err) => {
...
AlbumRouter.get('/artistId', (req, res, next) => {
  ...
  if (_id) {
    ...
    AlbumModel.find(
      ...
      .catch((error) =>
        res.send({ status: 0, msg: '获取专辑列表失败，请重新尝试' }),
      )
  } else {
...
AlbumRouter.get('/other', (req, res, next) => {
  ...
  AlbumModel.find(
    { $and: [{ singer_id: singerId }, { _id: { $ne: albumId } }] },
    { release_company: 0, album_introduction: 0 },
    { limit: 5 },
  )
    .then((album) => {
